...
class ind11c extends Model
{
    protected $guarded  =[];
    use HasFactory;
    public static function ind11c()
    {
        DB::statement(DB::raw('set @row:=0'));
        $ind11c=DB::select('select (@row:=@row + 1) AS NO,filename,substring(Field1,1,3) as Record_Type,substring(Field1,4,19) as PAN,substring(Field1,23,6) as Processing_Code,
        ...
        substring(Field1,139,15) as Card_Acceptor_Identification,substring(Field1,154,40) as Card_Acceptor_Name,
        ...
        substring(Field1,276,5) as Reserved from ind11cs');
        return $ind11c;
    }
}
