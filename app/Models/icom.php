...
class icom extends Model
{
    protected $guarded  =[];
    use HasFactory;
    public static function ICOM()
    {
        DB::statement(DB::raw('set @row:=0'));
        $ICOM=DB::select('select distinct substring(Field1,1,3) as recordtype,trim(substring(Field1,4,19)) as CardNo,substring(Field1,23,6) as process_code,
        ...
        format(concat(substring(Field1,303,10),".",substring(Field1,313,2)),2) as cardholder_fee,substring(Field1,315,10) as txn_tramsmission,
