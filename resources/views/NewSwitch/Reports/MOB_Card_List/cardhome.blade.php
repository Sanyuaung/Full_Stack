...
@section('content')
    ...
    <div class="mt-5 text-center">
        <h2><strong>MOB card List</strong>
        </h2>
    </div>
    <form action="{{ route('cardprint') }}" method="post">
        @csrf
        <div class="row mt-5">
            <div class="col">
            ...
            <div class="col">
                <label for="terminal">Select Card brand : </label>
                <select name="brand" id="terminal">
                    <option selected></option>
                    <option value="MOB_UPI_DB">MOB_UPI_DB</option>
                    <option value="MPU_DEBIT">MPU_DEBIT</option>
                </select>
                @error('brand')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
